...
export const createEmergencyRequest = async (req, res) => {
  try {
    ...
    // Removed 'reason' from required fields check
    ...
    const newRequest = new EmergencyBR({
      ...
      proofOfIdentificationNumber: proofOfIdentificationNumber,
...
export const validateEmergencyRequest = async (req, res) => {
  ...
  try {
    const request = await EmergencyBR.findByIdAndUpdate(req.params.id, { activeStatus: "Active" }, { new: true });
    if (!request) return res.status(404).json({ message: "Emergency request not found" });

    res.json({ message: "Emergency request deleted successfully" });
  }catch (error) {
    return res.status(500).json({ message: "Error validating emergency request", error });
  }
}
