...
export default function HospitalSignup() {
  ...
  return (
    <div className="flex min-h-screen items-center justify-center bg-cover bg-center p-6 bg-gray-900 bg-opacity-50 backdrop-blur-lg" style={{ backgroundImage: `url(${background})` }}>
      <Card className="w-full max-w-4xl p-10 shadow-2xl rounded-2xl bg-white bg-opacity-80 backdrop-blur-lg">
        ...
        <form className="grid grid-cols-1 md:grid-cols-2 gap-6" onSubmit={handleSubmit}>
          <div className="space-y-6">
            {["name", "city", "identificationNumber", "email", "phoneNumber"].map((id, index) => (
              <div key={index}>
                <Label htmlFor={id} value={id.replace(/([A-Z])/g, ' $1').trim()} className="text-gray-700 font-medium" />
                <TextInput id={id} type={id === "email" ? "email" : "text"} onChange={handleChange} required />
              </div>
            ))}
          </div>
          ...
          <div className="md:col-span-2 flex flex-col items-center space-y-4">
            ...
            <button onClick={() => navigate("/hospital_login")} className="text-red-600 font-medium hover:underline">Already have an account? Login</button>
          </div>
        </form>
      </Card>
    </div>
